...
class DAO:
    ...
    def __init__(self, collection_name: str):
        """Establish a data access object to a collection of the given name in the MongoDB
        database as specified in the environment variables. When the collection is first creted,
        it will be associated to a validator
        (see https://www.mongodb.com/docs/manual/core/schema-validation/)
        to ensure some basic data compliance.

        parameters:
            collection_name -- the name of the collection
            (a collection validator of the same name must be available)
        """
    ...
    def create(self, data: dict):
        ...
        The creation of a new document must comply to the corresponding validator, which 
        defines the data structure of the collection. In particular, the validator has 
        to make sure that: 
        ...
        (3) and the values of a property flagged with 'uniqueItems' 
            are unique among all documents of the collection.
